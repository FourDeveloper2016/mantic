...
public class Messages implements Serializable {
  ...
  private void load() {		
	...
	private void init() {
		...
		try {
			...
			for(String addMessage: atributos){
				String [] attrs= addMessage.split(Constantes.TILDE);
 				if(attrs.length> 1) {
					if(!attrs[1].equals(""))
						messages.put(attrs[0], new Message(attrs[0], attrs[1], new Properties()));
				} // if
			} // for
      ...
      messages.put(Constantes.NOMBRE_DE_APLICACION.toLowerCase(), message);
  ...
  public String getPropiedad(String proyecto, String key) {
    ...
    if(messages.containsKey(proyecto.toLowerCase()))
      regresar= messages.get(proyecto.toLowerCase()).getProperties().getProperty(key);
    else
